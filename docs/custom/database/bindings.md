...
Биндинги позволяют использовать MQTT-топики, публикуемые сторонними устройствами или сервисами, в качестве источников данных для устройств HOMEd Custom.
...
Шаблон для передачи состояния описываемого свойства в топик `outTopic`. В случае отсутствия этого параметра данные будут опубликованы без изменений.

Подробнее в разделе [шаблоны](#_2).

### `retain`

Если значение параметра равно `true`, сообщение в `outTopic` будет отправлено с флагом __retain__.
...
Этот элемент шаблона позволяет получать данные их строк, содержащих XML, по тегу. Парсер ищет первое вхождение указанного элемента в XML-строке и возвращает его текстовое содержимое. Вложенные теги и атрибуты не поддерживаются (возвращается только текст между открывающим и закрывающим тегами).

Например, если сообщение от устройства содержит данные:
```xml
<status>
  <power>on</power>
  <temperature>21.4</temperature>
  <humidity>55</humidity>
</status>
```

Для получения значения температуры нужно использовать следующий шаблон:
```
{{ xml.temperature }}
```

Специальные символы, такие как `&lt`, `&gt` или `&amp`, декодируются автоматически.
...
Данный элемент также может быть использован рекурсивно, что позволяет проверить несколько условий сразу:
```
...
Помимо описанных выше элементов, в шаблонах также могут быть использованы математические выражения. В этом случае соответствующая часть текста будет заменена числовым результатом вычислений. Например:
...
Список доступных функций можно посмотреть [здесь](https://github.com/u236/homed-service-common/blob/master/parser.cpp#L96-L103).
